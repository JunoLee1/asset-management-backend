...
export function setup() {
  ...
  const loanId = http.get(`${BASE_URL}/loans?page=1&pageSize=5`, {
    headers: { Authorization: `Bearer ${token}` },
  }).json('items.0.id')
  return { accessToken: token, loanId }
}

export default function (data) {
  const authHeaders = { headers: { Authorization: `Bearer ${data.accessToken}` } }

  const listRes = http.get(`${BASE_URL}/loans?page=1&pageSize=20`, {
    ...authHeaders,
    tags: { name: 'list' },
  })
  ...
  if (data.loanId) {
    const getRes = http.get(`${BASE_URL}/loans/${data.loanId}`, {
      ...authHeaders,
      tags: { name: 'getById' },
    })

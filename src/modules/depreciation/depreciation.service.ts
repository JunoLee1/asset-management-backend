...
import type {
  DepreciationDetail,
  DepreciationListItem,
  DepreciationRecordItem,
  ListDepreciationsParams,
  PaginatedDepreciations,
  UpsertDepreciationInput,
...
const requireManager = (requester: RequesterContext) => {
  if (requester.role !== 'ADMIN' && requester.role !== 'ASSET_MANAGER') {
    throw new AppError(403, '관리자 권한이 필요합니다.')
  }
}

// 연 상각률 자동 계산 (사용자 미입력 시)
const defaultAnnualRate = (method: DepreciationMethod, usefulLifeYears: number): number => {
  if (method === 'STRAIGHT_LINE') return 1 / usefulLifeYears
  // 정률법: 1 - (잔존가/취득가)^(1/N) 같은 방식도 있지만, 단순화 위해 STRAIGHT_LINE 의 2배 (200% 정률법)
  return Math.min(0.9999, 2 / usefulLifeYears)
}

// 연도별 감가상각 시뮬레이션 (records 자동 생성)
...
const simulate = (params: {
  ...
  annualRate: number
  startYear: number
  // 1=1월, 12=12월. 미지정/1월이면 기존(연도 기준) 동작과 동일.
  // 그 외에는 한국 법인세법 월할 계산: 첫해 (13-M)/12, 마지막 추가년도 (M-1)/12.
  startMonth?: number
}): YearlyRecord[] => {
  const {
    method,
  ...
  for (let i = 1; i <= totalYears; i++) {
    ...
    if (method === 'STRAIGHT_LINE') {
      ...
      if (bookValue - amount < salvageValue) amount = bookValue - salvageValue
    }
...
const getByAssetId = async (
  assetId: string,
  _requester: RequesterContext,
): Promise<DepreciationDetail | null> => {
  const row = await prisma.depreciation.findUnique({
    where: { assetId },
    include: {
  ...
  const records: DepreciationRecordItem[] = row.records.map((r) => ({
...
const upsert = async (
  input: UpsertDepreciationInput,
  requester: RequesterContext,
): Promise<DepreciationDetail> => {
  ...
  if (!asset.purchasePrice || !asset.purchaseDate) {
    throw new AppError(400, '자산에 구매가/구매일이 등록되어 있어야 감가상각을 적용할 수 있습니다.')
  }
  const purchasePrice = Number(asset.purchasePrice)
  if (input.salvageValue >= purchasePrice) {
    throw new AppError(400, '잔존가액은 구매가액보다 작아야 합니다.')
  }
  ...
  logger.info(
    { event: 'depreciation_upsert', assetId: input.assetId, method: input.method, records: simulated.length },
    '감가상각 등록/수정',
  )

  const result = await getByAssetId(input.assetId, requester)
  if (!result) throw new AppError(500, '조회 실패')
  return result
}

const remove = async (assetId: string, requester: RequesterContext): Promise<void> => {
  requireManager(requester)
  const existing = await prisma.depreciation.findUnique({ where: { assetId } })
  if (!existing) throw new AppError(404, '감가상각 설정을 찾을 수 없습니다.')

  const asset = await prisma.asset.findUnique({ where: { id: assetId }, select: { currentValue: true } })
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.assetHistory.create({
      data: {
        ...
        metadata: { event: 'depreciation_removed', lastBookValue } as object,
        performedById: requester.id,
      },
    })
  })

  logger.info({ event: 'depreciation_deleted', assetId }, '감가상각 삭제')
}

const list = async (params: ListDepreciationsParams): Promise<PaginatedDepreciations> => {
  const { q, method, page = 1, pageSize = 20 } = params
  const currentYear = new Date().getFullYear()

  const assetFilter = q
    ? { is: { status: { not: 'RETIRED' as const }, OR: [{ assetCode: { contains: q } }, { name: { contains: q } }] } }
    : { is: { status: { not: 'RETIRED' as const } } }
  const where = {
    asset: assetFilter,
    ...(method ? { method } : {}),
  }
  ...
  const [total, rows] = await Promise.all([
    ...
    prisma.depreciation.findMany({
      where,
      include: {
        asset: { select: { assetCode: true, name: true, purchasePrice: true } },
        records: {
          where: { fiscalYear: { lte: currentYear } },
  ...
  const items: DepreciationListItem[] = rows.map((row) => {
    const purchasePrice = row.asset.purchasePrice ? Number(row.asset.purchasePrice) : null
    const latestRecord = row.records[0]
    const currentBookValue = latestRecord ? Number(latestRecord.bookValue) : null
    const totalDepreciated =
      purchasePrice !== null && currentBookValue !== null ? purchasePrice - currentBookValue : null
...
export const depreciationService = { getByAssetId, list, upsert, remove, simulate, defaultAnnualRate }

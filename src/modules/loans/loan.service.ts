...
import { CONDITION_MAP, ACTION_MAP } from './loan.types'
import type {
  CreateLoanBody,
  ApproveLoanBody,
  RejectLoanBody,
...
const ALLOWED_TRANSITIONS: Record<LoanStatus, LoanStatus[]> = {
  ...
  INSPECTED: ['PENDING_RETURN_DEPT', 'PENDING_RETURN_ADMIN'],
  PENDING_RETURN_DEPT: ['PENDING_RETURN_ADMIN'],
...
type LoanWithRelations = Awaited<
  ReturnType<typeof prisma.loan.findFirstOrThrow<{ include: typeof LOAN_INCLUDE }>>
>
...
const toListItem = (row: LoanWithRelations): LoanListItem => ({
  id: row.id,
  status: row.status,
  assetId: row.assetId,
...
const toReturnDetail = (row: LoanWithRelations): LoanReturnDetail | null => {
  const r = row.loanReturn
  if (!r) return null // 반납의 기록이 없는 경우도 정상이기에 null
  return {
    id: r.id,
    ...
    conditionAfter: r.condition ? CONDITION_MAP[r.condition] : null,
    damageNote: r.damageNote,
    resultAction: r.condition ? ACTION_MAP[r.condition] : null,
...
const toDetail = (row: LoanWithRelations): LoanDetail => ({
  ...toListItem(row),
  rejectReason: row.rejectReason,
  recallReason: row.recallReason,
...
const assertTransition = (from: LoanStatus, to: LoanStatus): void => {
  if (!ALLOWED_TRANSITIONS[from].includes(to)) {
    throw new AppError(400, `${from} → ${to} 전이는 허용되지 않습니다.`)
  }
}

const assertNotSelfApprove = (loan: { userId: string }, requester: RequesterContext): void => {
  if (loan.userId === requester.id) {
    throw new AppError(403, '대여 승인/거절은 본인이 할 수 없습니다.')
  }
}
...
const isQueueStatus = (s?: LoanStatus): boolean =>
  s === 'PENDING_MANAGER' || s === 'PENDING_DEPT' || s === 'PENDING_ADMIN'

interface EdfMetrics {
  bucket: 0 | 1
  deadline: number
}

interface EdfLoanRow {
  createdAt: Date
  user: {
    hireDate: Date | null
    assignedAssets: Array<{
      maintenances: Array<{ estimatedCompletionDate: Date | null }>
    }>
  }
}

const computeEdfMetrics = (row: EdfLoanRow, now: number): EdfMetrics => {
  ...
  const etaList = row.user.assignedAssets
    ...
    .filter((t): t is number => t !== null) // type guard to be narrow number[]
  ...
  const bucket: 0 | 1 = isNewbie && hasInProgressMaint ? 0 : 1 // 0 = emergency, 1 = normal
...
const list = async (
...
): Promise<PaginatedResult<LoanListItem>> => {
  ...
  if (requester.role === 'DEPT_LEAD' || requester.role === 'REPAIR_OWNER') {
    ...
    andConditions.push({
      OR: [
        { user: { team: { departmentId: { in: deptIds } } } },
        { asset: { assignedUser: { team: { departmentId: { in: deptIds } } } } },
      ],
    })
  }
  if (query.keyword) {
  ...
  if (isQueueStatus(query.status)) {
    ...
    const sorted = rows
      ...
      .sort((a, b) => {
        if (a.metrics.bucket !== b.metrics.bucket) return a.metrics.bucket - b.metrics.bucket
...
const getById = async (id: string, requester: RequesterContext): Promise<LoanDetail> => {
  const row = await prisma.loan.findUnique({ where: { id }, include: LOAN_INCLUDE })
  if (!row) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (
    requester.role === 'USER' &&
...
const my = async (requester: RequesterContext): Promise<LoanListItem[]> => {
  const rows = await prisma.loan.findMany({
...
const overdue = async (requester: RequesterContext): Promise<LoanListItem[]> => {
  const where: Record<string, unknown> = {
...
const lookup = async (assetCode: string, requester: RequesterContext): Promise<LoanDetail> => {
  ...
  if (!row) {
    throw new AppError(404, '해당 자산의 수령 대기 중인 본인 대여를 찾을 수 없습니다.')
  }
  return toDetail(row)
}
...
const create = async (input: CreateLoanBody, requester: RequesterContext): Promise<LoanDetail> => {
  ...
  if (asset.class !== 'IT_ASSET') {
    throw new AppError(
      400,
      asset.class === 'FACILITY_ASSET'
        ? '시설 자산은 대여할 수 없습니다.'
        : '사무 자산은 대여할 수 없습니다. (담당자 배정만 가능)',
    )
  }
  if (asset.status !== 'IDLE') {
    throw new AppError(400, '대여 가능 상태가 아닌 자산입니다.')
  }
  if (asset.assignedUserId === requester.id) {
    throw new AppError(400, '이미 본인에게 할당된 자산입니다.')
  }
  // asset.status는 checkout 시점에야 IDLE에서 벗어나므로, 승인 대기/진행 중인
  // 다른 대여 건이 이미 있는 자산도 여기서 미리 걸러야 함 (그러지 않으면 두 건이
  // 독립적으로 승인 체인을 다 통과한 뒤 checkout 단계에서야 충돌이 드러남).
  const activeLoan = await prisma.loan.findFirst({
    where: {
      assetId: input.assetId,
      status: { notIn: ['RETURNED', 'REJECTED', 'CANCELLED', 'RECALLED'] },
    },
  })
  if (activeLoan) {
    throw new AppError(409, '이미 진행 중인 대여 신청이 있는 자산입니다.')
  }
  ...
  logger.info(
    { event: 'loan_created', id: created.id, assetId: input.assetId, userId: requester.id },
    '대여 신청',
  )
  ...
  try {
    ...
    await notificationService.createLoanRequestedNotifications({
      loanId: created.id,
    ...
    logger.warn(
      { event: 'loan_request_notify_failed', loanId: created.id, err },
      '대여 신청 알림 발송 실패',
    )
  }
...
const approveManager = async (
  ...
  if (!loan) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (requester.role === 'TEAM_LEAD') {
    const managedTeams = await prisma.team.findMany({
      where: { teamLeadId: requester.id },
      select: { id: true },
    ...
    if (!loan.user.teamId || !managedIds.includes(loan.user.teamId)) {
      throw new AppError(403, '담당 부서의 대여만 승인할 수 있습니다.')
    }
  }
  if (requester.role === 'ADMIN') {
    const mgr = loan.user.team?.teamLead
    if (mgr && !mgr.isOutOfOffice) {
      throw new AppError(403, '담당 팀장이 재직 중입니다. 팀장을 통해 승인하세요.')
    }
  }
  assertNotSelfApprove(loan, requester)
  if (loan.status !== 'PENDING_MANAGER') {
    throw new AppError(400, `PENDING_MANAGER 상태에서만 호출 가능 (현재: ${loan.status})`)
  }

  // CONTEXT.md L84 — PENDING_DEPT 스킵: DEPT_LEAD 미배정 / OOO / 신청자 본인이 부서장
  const deptLeaderId = loan.user.team?.department?.leaderId ?? null
  const deptLeaderOOO = loan.user.team?.department?.leader?.isOutOfOffice ?? false
  const skipDept = !deptLeaderId || deptLeaderOOO || deptLeaderId === loan.userId
  const nextStatus: LoanStatus = skipDept ? 'PENDING_ADMIN' : 'PENDING_DEPT'
  ...
  logger.info(
    { event: 'loan_approved_manager', id, by: requester.id, nextStatus },
    '대여 MANAGER 승인',
  )
  ...
  try {
    ...
    if (detail && nextStatus === 'PENDING_DEPT' && deptLeaderId) {
      await notificationService.createLoanPendingDeptNotification({
        loanId: id,
    ...
    } else if (detail && nextStatus === 'PENDING_ADMIN') {
      await notificationService.createLoanPendingAdminNotifications({
        loanId: id,
        assetCode: detail.asset.assetCode,
        assetName: detail.asset.name,
        applicantName: detail.user.name,
    ...
    logger.warn(
      { event: 'loan_post_manager_notify_failed', id, err },
      '관리자 승인 후 알림 실패',
    )
  }
...
const approveDept = async (
  ...
  if (loan.status !== 'PENDING_DEPT') {
    throw new AppError(400, `PENDING_DEPT 상태에서만 호출 가능 (현재: ${loan.status})`)
  }
  ...
  logger.info({ event: 'loan_approved_dept', id, by: requester.id }, '대여 DEPT 승인')
  ...
  try {
    ...
    logger.warn(
      { event: 'loan_pending_admin_notify_failed', id, err },
      '최종 승인 대기 알림 실패',
    )
  }
...
const approveAdmin = async (
  ...
  if (requester.role !== 'ASSET_MANAGER' && requester.role !== 'ADMIN') {
    throw new AppError(403, '자산담당자 또는 관리자 권한이 필요합니다.')
  }
  ...
  const requester_user = await prisma.user.findUnique({
    where: { id: loan.userId },
    select: { role: true },
  })
  if (requester_user?.role === 'ASSET_MANAGER' && requester.role !== 'ADMIN') {
    throw new AppError(403, '자산관리자 본인 대여는 ADMIN만 승인할 수 있습니다.')
  }

  if (loan.status !== 'PENDING_ADMIN') {
    throw new AppError(400, `PENDING_ADMIN 상태에서만 호출 가능 (현재: ${loan.status})`)
  }
  ...
  logger.info({ event: 'loan_approved_admin', id, by: requester.id }, '대여 ADMIN 승인')

  // T-AB: APPROVED 시점에 사원에게 알림 (X3 outbox — best effort)
  const assetInfo = await prisma.asset.findUnique({
...
const approveReturnManager = async (
  id: string,
  requester: RequesterContext,
): Promise<LoanDetail> => {
  if (requester.role !== 'TEAM_LEAD' && requester.role !== 'ADMIN') {
  ...
  if (requester.role === 'TEAM_LEAD') {
    ...
    if (!loan.user.teamId || !managedIds.includes(loan.user.teamId)) {
      throw new AppError(403, '담당 부서의 반납만 승인할 수 있습니다.')
    }
  }
  if (requester.role === 'ADMIN') {
    const mgr = loan.user.team?.teamLead
  ...
  if (loan.status !== 'INSPECTED') {
    throw new AppError(400, `INSPECTED 상태에서만 호출 가능 (현재: ${loan.status})`)
  }

  // ADR 0003 — PENDING_RETURN_DEPT 스킵 조건: 미배정·OOO·반납자 본인이 DEPT_LEAD
  const deptLeaderId = loan.user.team?.department?.leaderId ?? null
  const deptLeaderOOO = loan.user.team?.department?.leader?.isOutOfOffice ?? false
  const skipDeptReturn = !deptLeaderId || deptLeaderOOO || deptLeaderId === loan.userId
  const nextStatus = skipDeptReturn ? 'PENDING_RETURN_ADMIN' : 'PENDING_RETURN_DEPT'
  assertTransition(loan.status, nextStatus)

  await prisma.$transaction([
    prisma.loan.update({ where: { id }, data: { status: nextStatus } }),
    prisma.loanReturn.update({
      where: { loanId: id },
      data: { returnApprovedAt: new Date(), returnApprovedById: requester.id },
    }),
  ])
  logger.info(
    { event: 'loan_return_approved_manager', id, by: requester.id, nextStatus },
    '반납 MANAGER 승인',
  )

  if (nextStatus === 'PENDING_RETURN_DEPT' && deptLeaderId) {
    try {
      const detail = await prisma.loan.findUnique({
        where: { id },
        include: {
      ...
      if (detail) {
        await notificationService.createLoanPendingReturnDeptNotification({
          loanId: id,
          assetCode: detail.asset.assetCode,
          assetName: detail.asset.name,
          returnerName: detail.user.name,
          deptLeaderId,
        })
      }
    } catch (err) {
      logger.warn({ event: 'loan_return_dept_notify_failed', id, err }, '반납 부서장 확인 알림 실패')
    }
  }
...
// 반납 DEPT_LEAD 확인 — PENDING_RETURN_DEPT → PENDING_RETURN_ADMIN (ADR 0003)
// 확인만, 반려 없음. ADMIN 은 DEPT_LEAD OOO 시 대행
const approveReturnDept = async (
  id: string,
  requester: RequesterContext,
): Promise<LoanDetail> => {
  if (requester.role !== 'DEPT_LEAD' && requester.role !== 'REPAIR_OWNER' && requester.role !== 'ADMIN') {
  ...
  if (requester.role === 'DEPT_LEAD' || requester.role === 'REPAIR_OWNER') {
    if (deptLeaderId !== requester.id) {
      throw new AppError(403, '담당 부서의 반납만 확인할 수 있습니다.')
    }
  }
  if (requester.role === 'ADMIN' && deptLeaderId && !deptLeaderOOO) {
    throw new AppError(403, '담당 부서장이 재직 중입니다. 부서장을 통해 확인하세요.')
  }
  if (loan.status !== 'PENDING_RETURN_DEPT') {
    throw new AppError(400, `PENDING_RETURN_DEPT 상태에서만 호출 가능 (현재: ${loan.status})`)
  }
  assertTransition(loan.status, 'PENDING_RETURN_ADMIN')

  await prisma.$transaction([
    prisma.loan.update({ where: { id }, data: { status: 'PENDING_RETURN_ADMIN' } }),
    prisma.loanReturn.update({
      where: { loanId: id },
      data: { deptReturnApprovedAt: new Date(), deptReturnApprovedById: requester.id },
    }),
  ])
  logger.info({ event: 'loan_return_approved_dept', id, by: requester.id }, '반납 DEPT 확인')
  return getById(id, requester)
}

// 반납 최종 — PENDING_RETURN_ADMIN → RETURNED + Asset 갱신 + 이력
const finalizeReturn = async (id: string, requester: RequesterContext): Promise<LoanDetail> => {
  if (requester.role !== 'ASSET_MANAGER' && requester.role !== 'ADMIN') {
    throw new AppError(403, '자산담당자 또는 관리자 권한이 필요합니다.')
  }
  const loan = await prisma.loan.findUnique({ where: { id } })
  ...
  if (loan.status !== 'PENDING_RETURN_ADMIN') {
    throw new AppError(400, `PENDING_RETURN_ADMIN 상태에서만 호출 가능 (현재: ${loan.status})`)
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (!lr || !lr.condition) {
      throw new AppError(500, '검수 결과가 없습니다.')
    }
    const newAssetCondition = CONDITION_MAP[lr.condition]
    const isLost = lr.condition === 'LOST'
    const needsRepair = ACTION_MAP[lr.condition] === 'NEEDS_REPAIR'
  ...
  logger.info({ event: 'loan_returned', id, by: requester.id }, '반납 ADMIN 회수')
  return getById(id, requester)
}
...
const checkout = async (
  ...
  await prisma.$transaction(async (tx) => {
    // 자산이 여전히 IDLE일 때만 원자적으로 점유 — 동일 자산에 두 대여 건이
    // 독립적으로 승인 체인을 통과해 checkout까지 온 경우, 두 번째 checkout이
    // 자산을 조용히 재배정해버리는 것을 막는 게이트.
    const claimed = await tx.asset.updateMany({
      where: { id: loan.assetId, status: 'IDLE' },
      data: { status: 'OPERATING', assignedUserId: loan.userId },
    })
    if (claimed.count === 0) {
      throw new AppError(409, '자산이 이미 다른 대여 건으로 출고되었거나 대여 가능한 상태가 아닙니다.')
    }
  ...
  logger.info({ event: 'loan_checked_out', id, by: requester.id }, '대여 출고')
  // T-AB: CHECKED_OUT 시점에 사원에게 알림 (X3 outbox — best effort)
  const checkoutAssetInfo = await prisma.asset.findUnique({
    where: { id: loan.assetId },
    select: { assetCode: true, name: true },
  })
...
const receive = async (id: string, requester: RequesterContext): Promise<LoanDetail> => {
  const loan = await prisma.loan.findUnique({
    where: { id },
  ...
  if (!loan) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (loan.userId !== requester.id) {
    throw new AppError(403, '본인이 대여한 건만 수령 확정할 수 있습니다.')
  }
  if (loan.status !== 'CHECKED_OUT') {
    throw new AppError(400, 'CHECKED_OUT 상태의 대여만 수령 확정할 수 있습니다.')
  }
  ...
  logger.info({ event: 'loan_received', id, by: requester.id }, '대여 수령 확정')
...
const reject = async (
  ...
  if (!ALLOWED_TRANSITIONS[loan.status].includes('REJECTED')) {
    throw new AppError(400, `현재 상태(${loan.status})에서는 거절할 수 없습니다.`)
  }
  ...
  logger.info({ event: 'loan_rejected', id, by: requester.id }, '대여 거절')
  return getById(id, requester)
}

const cancel = async (id: string, requester: RequesterContext): Promise<LoanDetail> => {
  const loan = await prisma.loan.findUnique({ where: { id } })
  if (!loan) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (loan.userId !== requester.id) {
    throw new AppError(403, '본인이 신청한 대여만 취소할 수 있습니다.')
  }
  if (loan.status !== 'PENDING_MANAGER') {
  ...
  logger.info({ event: 'loan_cancelled', id, by: requester.id }, '대여 취소')
  return getById(id, requester)
}
...
const recall = async (
  ...
  if (loan.status !== 'CHECKED_OUT' && loan.status !== 'RECEIVED') {
    throw new AppError(400, 'CHECKED_OUT 또는 RECEIVED 상태의 대여만 강제 회수할 수 있습니다.')
  }
  ...
  logger.info({ event: 'loan_recalled', id, by: requester.id }, '대여 강제 회수')
  return getById(id, requester)
}
...
const requestReturn = async (id: string, requester: RequesterContext): Promise<LoanDetail> => {
  const loan = await prisma.loan.findUnique({
    where: { id },
    include: { asset: { select: { assetCode: true, name: true } } },
  })
  if (!loan) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (loan.userId !== requester.id) {
    throw new AppError(403, '본인이 대여한 건만 반납 요청할 수 있습니다.')
  }
  // R-1: RECEIVED 상태에서만 반납 요청 (CHECKED_OUT 은 미수령 상태라 반납 의미 X)
  if (loan.status !== 'RECEIVED') {
    throw new AppError(400, '기기 수령 시에만 반납 요청할 수 있습니다.')
  }
  ...
  logger.info({ event: 'loan_return_requested', id, by: requester.id }, '반납 요청')
  ...
  try {
    ...
    await notificationService.createLoanReturnRequestedNotifications({
      loanId: id,
      assetCode: loan.asset.assetCode,
      assetName: loan.asset.name,
    ...
    logger.warn(
      { event: 'loan_return_notify_failed', loanId: id, err },
      '반납 검수 대기 알림 발송 실패',
    )
  }
...
const inspect = async (
  ...
  if (loan.status !== 'PENDING_INSPECTION') {
    throw new AppError(400, 'PENDING_INSPECTION 상태의 대여만 검수할 수 있습니다.')
  }
  ...
  const needsRepair = ACTION_MAP[body.condition] === 'NEEDS_REPAIR'
...
const requestExtension = async (id: string, days: number, requester: RequesterContext) => {
  const loan = await prisma.loan.findUnique({ where: { id } })
  if (!loan) throw new AppError(404, '대여 건을 찾을 수 없습니다.')
  if (loan.userId !== requester.id) throw new AppError(403, '본인 대여만 연장 신청할 수 있습니다.')
  if (!['CHECKED_OUT', 'RECEIVED'].includes(loan.status))
    throw new AppError(400, '대여 중인 상태에서만 연장 신청할 수 있습니다.')
  if (days < 1 || days > 365) throw new AppError(400, '연장 기간은 1일 이상 365일 이하여야 합니다.')
  return prisma.loanExtension.create({
    data: { loanId: id, days, requestedById: requester.id, status: 'PENDING_MANAGER' },
  })
}

const approveExtensionManager = async (extensionId: string, requester: RequesterContext) => {
  const ext = await prisma.loanExtension.findUnique({
    where: { id: extensionId },
    include: { loan: { include: { asset: { include: { department: true } } } } },
  })
  if (!ext) throw new AppError(404, '연장 신청을 찾을 수 없습니다.')
  if (ext.status !== 'PENDING_MANAGER') throw new AppError(400, '팀장 승인 대기 상태가 아닙니다.')
  if (requester.role === 'TEAM_LEAD') {
    const managedTeams = await prisma.team.findMany({
      where: { teamLeadId: requester.id },
    ...
    if (!deptIds.includes(ext.loan.asset.departmentId))
      throw new AppError(403, '자산 소속 부서의 팀장만 승인할 수 있습니다.')
  }
  return prisma.loanExtension.update({
    where: { id: extensionId },
    data: {
      status: 'PENDING_ADMIN',
      managerApprovedById: requester.id,
...
const approveExtensionAdmin = async (extensionId: string, requester: RequesterContext) => {
  if (requester.role !== 'ADMIN') throw new AppError(403, '관리자 권한이 필요합니다.')
  const ext = await prisma.loanExtension.findUnique({
    where: { id: extensionId },
    include: { loan: true },
  })
  if (!ext) throw new AppError(404, '연장 신청을 찾을 수 없습니다.')
  if (ext.status !== 'PENDING_ADMIN') throw new AppError(400, '관리자 승인 대기 상태가 아닙니다.')
  ...
  return prisma.$transaction([
    ...
    prisma.loan.update({ where: { id: ext.loanId }, data: { dueDate: newDue } }),
  ])
}
...
const rejectExtension = async (
  ...
  const ext = await prisma.loanExtension.findUnique({ where: { id: extensionId } })
  if (!ext) throw new AppError(404, '연장 신청을 찾을 수 없습니다.')
  if (!['PENDING_MANAGER', 'PENDING_ADMIN'].includes(ext.status))
    throw new AppError(400, '승인 대기 상태가 아닙니다.')
  return prisma.loanExtension.update({
    where: { id: extensionId },
    data: {
      status: 'REJECTED',
...
const notifyOverdueLoans = async (): Promise<{ notified: number }> => {
  const now = new Date()
  let notified = 0

...
export function setup() {
  ...
  const maintenanceId = http.get(`${BASE_URL}/maintenances?page=1&pageSize=5`, {
    headers: { Authorization: `Bearer ${token}` },
  }).json('items.0.id')
  return { accessToken: token, maintenanceId }
}

export default function (data) {
  const authHeaders = { headers: { Authorization: `Bearer ${data.accessToken}` } }

  const listRes = http.get(`${BASE_URL}/maintenances?page=1&pageSize=20`, {
    ...authHeaders,
    tags: { name: 'list' },
  })
  ...
  if (data.maintenanceId) {
    const getRes = http.get(`${BASE_URL}/maintenances/${data.maintenanceId}`, {
      ...authHeaders,
      tags: { name: 'getById' },
    })

...
import type { AssetClass, AssetStatus } from '../../generated/prisma/enums'
import type {
  DistributionData,
  UtilizationData,
  DepartmentValueData,
...
const calculateAssetUtilizationWithWeights = async (assetId: string): Promise<number> => {
  ...
  const initialStatus = (histories[0].metadata as { from?: string })?.from || asset.status
  const initialWeight = STATUS_WEIGHTS[initialStatus as AssetStatus] ?? 0
  ...
  for (let i = 0; i < histories.length; i++) {
    ...
    const statusAtThisChange = (currentHistory.metadata as { to?: string })?.to || asset.status
    const weight = STATUS_WEIGHTS[statusAtThisChange as AssetStatus] ?? 0
...
const getDistribution = async (): Promise<DistributionData> => {
  const [byStatusRaw, byConditionRaw, byClassRaw] = await Promise.all([
    prisma.asset.groupBy({ by: ['status'], where: { status: { not: 'RETIRED' } }, _count: true }),
    prisma.asset.groupBy({
      by: ['condition'],
      where: { status: { not: 'RETIRED' } },
      _count: true,
    }),
    prisma.asset.groupBy({ by: ['class'], where: { status: { not: 'RETIRED' } }, _count: true }),
  ])
...
const getUtilization = async (): Promise<UtilizationData> => {
  ...
  const [activeOpsCount, totalRegisteredCount] = await Promise.all([
    prisma.asset.count({ where: { status: 'OPERATING' } }),
    prisma.asset.count({
      where: {
...
const getDepartmentValue = async (): Promise<DepartmentValueData> => {
...
const getUtilizationByDepartment = async (
  assetClass?: AssetClass,
): Promise<DepartmentUtilizationData> => {
  const REGISTERED_STATUSES = ['OPERATING', 'IDLE', 'STANDBY', 'REPAIR', 'PENDING_DISPOSAL'] as const
  ...
  return {
    ...
    overall: {
      ...
      utilizationRatePct:
        overallTotal > 0 ? Math.round((overallActive / overallTotal) * 1000) / 10 : 0,
    },
  }
}

const getMaintenanceCost = async (from?: string, to?: string): Promise<MaintenanceCostData> => {
  ...
  const assetMap = new Map<
    string,
    { assetCode: string; name: string; purchasePrice: number; totalCost: number; count: number }
  >()
  ...
  const topAssets = Array.from(assetMap.entries())
    .map(([assetId, data]) => ({
      ...
      costRatio:
        data.purchasePrice > 0 ? Math.round((data.totalCost / data.purchasePrice) * 1000) / 10 : 0,
...
const getComplianceExpiry = async (): Promise<ComplianceData> => {
  const now = new Date()
  ...
  const allLicensesForOverseated = await prisma.license.findMany({
  ...
  const warrantyExpiry = hardwareAssets.map((hw) => ({
    ...
    daysLeft: Math.ceil(
      (new Date(hw.warrantyEnd!).getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
    ),
  }))
  ...
  const licenseExpiry = licenses.map((lic) => ({
    ...
    daysLeft: Math.ceil(
      (new Date(lic.expiryDate!).getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
    ),

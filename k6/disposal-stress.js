...
export function setup() {
  ...
  const disposalId = http.get(`${BASE_URL}/disposals?page=1&pageSize=5`, {
    headers: { Authorization: `Bearer ${token}` },
  }).json('items.0.id')
  return { accessToken: token, disposalId }
}

export default function (data) {
  const authHeaders = { headers: { Authorization: `Bearer ${data.accessToken}` } }

  const listRes = http.get(`${BASE_URL}/disposals?page=1&pageSize=20`, {
    ...authHeaders,
    tags: { name: 'list' },
  })
  ...
  if (data.disposalId) {
    const getRes = http.get(`${BASE_URL}/disposals/${data.disposalId}`, {
      ...authHeaders,
      tags: { name: 'getById' },
    })

...
async function exportCsv(ctx) {
    const [dist, util, dept, maint, comp] = await Promise.all([
        analytics_service_1.analyticsService.getDistribution(),
        analytics_service_1.analyticsService.getUtilization(),
        analytics_service_1.analyticsService.getDepartmentValue(),
        analytics_service_1.analyticsService.getMaintenanceCost(),
        analytics_service_1.analyticsService.getComplianceExpiry(),
    ]);
    const rows = [];
    ...
    util.assets.filter((a) => a.isIdle).slice(0, 10).forEach((a) => rows.push([
    ...
    dept.departments.forEach((d) => rows.push([d.name, String(d.assetCount), String(d.totalValue), String(d.memberCount), String(d.valuePerMember)]));
    rows.push(['합계', '', String(dept.totalValue), '', '']);
    rows.push([]);
    rows.push(['[정비 비용 — TOP 20]']);
    rows.push(['자산코드', '자산명', '구매가(원)', '누적 정비비(원)', '구매가 대비(%)', '정비 횟수']);
    maint.topAssets.forEach((a) => rows.push([a.assetCode, a.name, String(a.purchasePrice), String(a.totalCost), String(a.costRatio), String(a.maintenanceCount)]));
    ...
    comp.licenseExpiry.forEach((l) => rows.push([l.name, fmtDate(l.expiryDate), `D-${l.daysLeft}`, String(l.seatsTotal), String(l.seatsUsed)]));
    rows.push([]);
    if (comp.overseated.length > 0) {
        rows.push(['[컴플라이언스 위반 — 시트 초과]']);
        rows.push(['라이선스명', '계약 시트', '사용 시트', '초과 수']);
        comp.overseated.forEach((o) => rows.push([o.name, String(o.seatsTotal), String(o.seatsUsed), String(o.seatsUsed - o.seatsTotal)]));
    }
...
async function exportExcel(ctx) {
    ...
    const WARN_FILL = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } };
...
async function exportHwpx(ctx) {
  ...
  <hp:p paraPrIDRef="1" instId="${paraId++}"><hp:run><hp:charPr bold="${isHeader}"/><hp:t>${escXml(text)}</hp:t></hp:run></hp:p>
    ...
    const sections = [
        ...
        tbl(['자산코드', '자산명', '카테고리', '가동률(%)', '마지막 사용일'], util.assets.filter((a) => a.isIdle).slice(0, 10).map((a) => [
            a.assetCode, a.name, a.categoryName, `${a.utilizationPct}%`,
            a.lastUsedAt ? fmtDate(a.lastUsedAt) : '없음',
        ])),
        p(''),
        ...
        tbl(['부서명', '자산 수', '총 가치(원)', '인원 수', '인당 가치(원)'], dept.departments.map((d) => [
            d.name, String(d.assetCount), fmtKRW(d.totalValue), String(d.memberCount),
            d.memberCount > 0 ? fmtKRW(d.valuePerMember) : '–',
        ])),
        p(''),
        ...
        tbl(['자산코드', '자산명', '구매가(원)', '누적 정비비(원)', '구매가 대비(%)', '정비 횟수'], maint.topAssets.slice(0, 15).map((a) => [
...
//# sourceMappingURL=analytics.export.js.map